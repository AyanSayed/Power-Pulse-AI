...
export default function LiveMeter() {
  ...
  return <div className="p-6 md:p-8 w-full space-y-6">
    ...
    <div className="grid md:grid-cols-2 gap-6">
      <section className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-6 transition-all duration-300 ${flashPower ? "ring-2 ring-amber-300" : ""}`}><p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Current power</p><p className="text-4xl font-extrabold text-emerald-600 mt-2">{totalPower ?? 0}<span className="text-xl ml-1">W</span></p></section>
    ...
    <section className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-5 border-b border-gray-100"><h2 className="font-semibold text-gray-900">Meter reading history</h2><p className="text-sm text-gray-500 mt-1">Your {HISTORY_LIMIT} most recent whole-house power readings.</p></div>
      {history.length ? <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-gray-50 text-gray-500"><tr><th className="px-6 py-3 font-medium">Time</th><th className="px-6 py-3 font-medium">Power</th></tr></thead><tbody>{history.map((reading) => <tr key={reading._id} className="border-t border-gray-100"><td className="px-6 py-3 text-gray-600">{reading.receivedAt ? new Date(reading.receivedAt).toLocaleString() : "Unknown"}</td><td className="px-6 py-3 font-semibold text-gray-900">{totalWatts(reading)} W</td></tr>)}</tbody></table></div> : <p className="px-6 py-8 text-sm text-gray-500">No meter readings are available yet. Connect the meter to start recording history.</p>}
    </section>
  </div>;
}
