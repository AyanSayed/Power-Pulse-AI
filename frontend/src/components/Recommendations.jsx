  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 h-full">
      ...
      {visible.length > 0 ? (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-500 mt-1 leading-6">{item.description}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <p className="text-sm text-gray-500">Upload a bill to unlock personalized recommendations.</p>
      )}

      {showViewAll && recommendations.length > 0 && (
        <ViewMoreButton to="/ai-insights/recommendations" label="View All Recommendations" accent="amber" />
      )}
    </div>
  );
}
