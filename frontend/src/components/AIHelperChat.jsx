...
function AIHelperChat() {
  ...
  async function send() {
    ...
    setMessages((items) => [...items, { role: "user", text: question }]); setInput("");
    const destination = NAVIGATION.find((item) => item.match.test(question));
    if (destination) { setMessages((items) => [...items, { role: "assistant", text: `Open ${destination.label}?`, route: destination.route, label: destination.label }]); return; }
    setLoading(true);
    try { const response = await apiClient.post("/api/assistant", { question, language }); setMessages((items) => [...items, { role: "assistant", text: response.data.answer }]); }
    catch { setMessages((items) => [...items, { role: "assistant", text: "I couldn't reach the guide right now. Ask about uploads, budget, slabs, or bill history." }]); }
